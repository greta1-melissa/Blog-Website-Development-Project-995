...
const Admin = () => {
  // MASTER LIST: use 'posts' which contains all rows from DB unfiltered
  const { posts = [], categories = [], deletePost, updatePost } = useBlog();
  const { kdramas = [], addKdrama, updateKdrama, deleteKdrama } = useKdrama();
  const { user, isAdmin, logout } = useAuth();
  const [activeTab, setActiveTab] = useState('posts'); // Default to posts for management

  // Filtering State (UI only)
  ...
  const filteredPosts = useMemo(() => {
    // ADMIN REQUIREMENT: 
    // Start with the full list. Deduplicate if necessary.
    const uniquePosts = Array.from(new Map(posts.map(p => [String(p.id), p])).values());

    // Filter ONLY by Search, Category, and Status. 
    // isHandPicked is NOT a filter here.
  ...
  const getStatusBadge = (status) => {
    ...
    switch (s) {
      case 'published':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><SafeIcon icon={FiCheckCircle} className="mr-1" /> Published</span>;
      case 'draft':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800"><SafeIcon icon={FiFile} className="mr-1" /> Draft</span>;
      case 'scheduled':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800"><SafeIcon icon={FiClock} className="mr-1" /> Scheduled</span>;
      default: return null;
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600">Managing {stats.totalPosts} total stories</p>
        </div>
                  ...
                  <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-3">
                        ...
                        <button 
                          onClick={() => { if(window.confirm('Delete post?')) deletePost(post.id); }} 
      ...
      {activeTab === 'kdramas' && (
        <div className="bg-white rounded-lg p-12 text-center text-gray-500 border border-dashed">
          K-Drama management is available in the secondary tab.
        </div>
      )}
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            <h3 className="text-lg font-bold mb-2">Total Stories</h3>
            <p className="text-4xl font-bold text-purple-600">{stats.totalPosts}</p>
          </div>
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            <h3 className="text-lg font-bold mb-2">Featured Stories</h3>
