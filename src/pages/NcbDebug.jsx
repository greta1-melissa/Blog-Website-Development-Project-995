...
const NcbDebug = () => {
  ...
  const runProxyTest = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setProxyTestResult({ 
          status: 'success', 
          message: `Success! Read ${data.length} records. Proxy is working.` 
        });
      } else {
      ...
      setProxyTestResult({ 
        status: 'error', 
        message: `Proxy Test Failed: ${e.message}` 
      });
    }
  };
  ...
  const runWriteTest = async () => {
    ...
    try {
      const testPayload = { title: "Proxy Write Test", content: "Test", category: "Debug", status: "draft" };
      const createdPost = await ncbCreate('posts', testPayload);
      if (!createdPost || !createdPost.id) throw new Error("Created post has no ID");
  ...
  const StatusItem = ({ label, value, icon: Icon }) => (
    <div className="flex justify-between items-center py-3 border-b border-gray-100 last:border-0">
      <div className="flex items-center text-gray-600 font-medium">
        {Icon && <SafeIcon icon={Icon} className="mr-2 text-gray-400" />} {label}
      </div>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-gray-700 font-medium flex items-center">
                      <SafeIcon icon={FiCloudLightning} className="mr-2 text-blue-500" /> Proxy Read (K-Dramas)
                    </span>
                  ...
                  {proxyTestResult && (
                    <div className={`mt-2 text-sm p-3 rounded-lg ${
                      proxyTestResult.status === 'success' ? 'bg-green-50 text-green-800' : 
                      proxyTestResult.status === 'loading' ? 'bg-gray-50 text-gray-600' : 'bg-red-50 text-red-800'
                    }`}>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                   <div className="flex justify-between items-center mb-2">
                    <span className="text-gray-700 font-medium flex items-center">
                      <SafeIcon icon={FiDatabase} className="mr-2 text-purple-500" /> Proxy Write (Create/Delete)
                    </span>
                  ...
                  {writeTestStatus && (
                    <div className={`mt-2 text-sm p-3 rounded-lg ${
                      writeTestStatus === 'success' ? 'bg-green-50 text-green-800' : 
                      writeTestStatus === 'error' ? 'bg-red-50 text-red-800' : 'bg-blue-50 text-blue-800'
                    }`}>
                      {writeTestStatus === 'success' ? 'Write test passed (Created & Deleted).' : writeError}
                    </div>
                  )}
                </div>
              </div>
