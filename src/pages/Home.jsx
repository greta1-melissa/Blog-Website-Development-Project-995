...
import { KDRAMA_PLACEHOLDER, BLOG_PLACEHOLDER } from '../config/assets';

const { FiTv, FiArrowRight, FiCalendar, FiStar, FiCoffee, FiBookOpen, FiSun, FiMoon, FiShoppingBag, FiHeart } = FiIcons;
...
const Home = () => {
  ...
  // Filter for Product Recommendations
  ...
  const featuredPosts = useMemo(() => {
    ...
    let selection = posts.filter(p => isHandpicked(p) && p.category !== 'Product Recommendations');
    selection.sort((a, b) => getPostTime(b) - getPostTime(a));

    if (selection.length < 3) {
      const selectedIds = new Set(selection.map(p => p.id));
      const fillers = posts
        .filter(p => !selectedIds.has(p.id) && p.category !== 'Product Recommendations')
  ...
  const meTimeRituals = [
    ...
    },
    {
      ...
      desc: "Washing off the day and layering on the calm. My non-negotiable.",
      icon: FiMoon,
      color: "bg-blue-50 text-blue-600",
      image: "https://images.unsplash.com/photo-1556228720-195a672e8a03?w=600&fit=crop"
    },
    {
    ...
    },
    {
      ...
      desc: "Greeting the day with a gentle flow and deep breaths.",
      icon: FiSun,
      color: "bg-yellow-50 text-yellow-600",
      image: "https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=600&fit=crop"
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-50/50 via-white to-purple-50 pb-24">
      ...
      {/* Section: Self-Care & Me Time */}
      <div className="bg-purple-50/50 py-24 border-y border-purple-100/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div className="max-w-xl">
              <div className="flex items-center gap-2 mb-2">
                <SafeIcon icon={FiHeart} className="text-purple-600" />
                <span className="text-xs font-bold text-purple-600 uppercase tracking-widest">Personal Rituals</span>
              </div>
              <h2 className="text-4xl font-serif font-bold text-gray-900 mb-4">Small Joys: Self-Care &amp; Me Time</h2>
              <p className="text-gray-600 text-lg leading-relaxed">
                Life as a mom can be non-stop. Here are the little rituals and quiet moments I carve out to stay grounded and find my own "Magic Shop."
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {meTimeRituals.map((ritual, index) => (
              ...
              >
                ...
                <div className="p-8">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${ritual.color}`}>
      ...
      {productRecs.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <SafeIcon icon={FiShoppingBag} className="text-purple-600" />
                <span className="text-xs font-bold text-purple-600 uppercase tracking-widest">Melissa's Favorites</span>
              </div>
              <h2 className="text-4xl font-serif font-bold text-gray-900">Mom-Approved Finds</h2>
              <p className="text-gray-600 mt-2">Genuinely loved items from skincare to BTS merch.</p>
            </div>
            <Link
              to="/products"
              className="mt-6 md:mt-0 font-bold text-purple-600 hover:text-purple-800 flex items-center group"
            >
              Shop All Picks <SafeIcon icon={FiArrowRight} className="ml-2 group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 bg-white border-y border-purple-50">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts && posts.filter(p => p.category !== 'Product Recommendations').slice(0, 3).map((post, index) => (
      ...
      <div className="bg-purple-50/20 py-24 border-b border-purple-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
