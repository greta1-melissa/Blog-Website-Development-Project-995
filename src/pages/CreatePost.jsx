...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      const postId = await addPost({ ...formData, image: cleanImageInput, author: user?.name || 'BangtanMom' });
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto px-4 py-12">
        ...
        {errorMessage && (
          <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start">
            ...
            <div className="text-red-800 font-medium"><p>Error:</p><p className="text-sm font-normal">{errorMessage}</p></div>
          </div>
        )}
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-xl shadow-sm p-8 border border-purple-50">
              <div className="mb-6">
                ...
                <input type="text" name="title" value={formData.title} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg" required />
              </div>
              <div className="mb-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-purple-50 p-6">
              ...
              <div className="space-y-4">
                ...
                <button type="submit" disabled={isSaving} className="w-full flex items-center justify-center px-4 py-3 bg-purple-600 text-white font-bold rounded-lg disabled:opacity-70 shadow-lg">
                  {isSaving ? <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent mr-2" /> : <><SafeIcon icon={FiSave} className="mr-2" /> {formData.status === 'draft' ? 'Save Draft' : 'Save Post'}</>}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-purple-50 p-6">
              ...
              <div className="space-y-4">
                <input type="url" name="image" value={formData.image} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Image URL..." />
                ...
                {formData.image && (
                  <div className="relative rounded-lg overflow-hidden h-32 w-full border border-gray-200">
                    <img src={getImageSrc(formData.image)} alt="Preview" className={`w-full h-full object-cover ${imageError ? 'opacity-0' : 'opacity-100'}`} onError={() => setImageError(true)} onLoad={() => setImageError(false)} />
                  </div>
                )}
              </div>
            </div>
